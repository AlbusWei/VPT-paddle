...
class ResNet50(nn.Layer):
    def __init__(self, in_dim=64, num_classes=1000):
        super().__init__()
        self.in_dim = in_dim
        self.conv1 = nn.Conv2D(in_channels=3,out_channels=in_dim,kernel_size=7,stride=2,padding=3,bias_attr=False)
        self.bn1 = nn.BatchNorm2D(in_dim)
        self.MaxPool = nn.MaxPool2D(kernel_size=3,stride=2,padding=1)
        self.relu = nn.ReLU()
        self.layer1 = self._make_layer(dim=64, n_blocks=3,stride=1)
        self.layer2 = self._make_layer(dim=128, n_blocks=4,stride=2)
        self.layer3 = self._make_layer(dim=256, n_blocks=6,stride=2)
        self.layer4 = self._make_layer(dim=512, n_blocks=3,stride=2)
