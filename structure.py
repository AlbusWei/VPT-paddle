...
from ViTmodel.SwinT import SwinTransformer
...
class VPT_ViT(VisionTransformer):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, class_num=1000, embed_dim=768, depth=12,
        ...
        if VPT_type == "Deep":
            self.Prompt_Tokens = paddle.static.create_parameter([depth, Prompt_Token_num, embed_dim], dtype='float32')
        else:  # "Shallow"
            self.Prompt_Tokens = paddle.static.create_parameter([1, Prompt_Token_num, embed_dim], dtype='float32')
...
class VPT_SwinT(SwinTransformer):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, class_num=1000, embed_dim=768, depth=12,
                 num_heads=8, mlp_ratio=4., qkv_bias=True, window_size=7, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 Prompt_Token_num=1, VPT_type="Shallow", basic_state_dict=None):

        # Recreate ViT
        super().__init__(img_size, patch_size, in_chans, class_num, embed_dim, depth, num_heads, window_size,mlp_ratio,
                         qkv_bias, qk_scale,
                         drop_rate=drop_rate,
                         attn_drop_rate=attn_drop_rate,
                         drop_path_rate=drop_path_rate,
